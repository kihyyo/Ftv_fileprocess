...
class LogicNormal(object):
    ...
    def item_list(path, f):
        try:
            ...
            item['path'] = path
            try:
                item['year'] = guessit(item['path'])['year']
            except:
                item['year'] = None
                ...
                if not LogicNormal.isHangul(tmdb['data']['title']) > 0:
                    ...
                    try: 
                        ...
                        if watcha['ret'] == 'empty':
                            ...
                            if LogicNormal.isHangul(watcha['data'][0]['title']) > 0 :
                                tmdb['data']['title'] = watcha['data'][0]['title']
                                logger.debug('한글 제목: %s', tmdb["data"]["title"])
                            else:
                                logger.debug('검색 실패')
                                pass
                    except Exception as e:
                    ...
                    if tmdb['data']['country'] == []:
                        ...
                        watcha = SiteWatchaTv.search(keyword, year)
                        if watcha['ret'] == 'empty':
