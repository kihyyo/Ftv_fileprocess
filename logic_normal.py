...
class LogicNormal(object):
        ...
        try:
            ...
            if LogicFtv.search((), en_title, year) != []:
                if year is not None:
                    ...
                    tmdb_code = LogicFtv.search((), en_title)[0]['code'].strip()
                tmdb = SiteTmdbFtv.info(tmdb_code)
                logger.debug('TMDB 제목: %s', tmdb["data"]["title"])
                if tmdb['data']['country'] == []:
                    keyword = tmdb['data']['title'].strip()
                    watcha = SiteWatchaTv.search(keyword)
                    if watcha['ret'] == 'empty':
                        tmdb['data']['country'] == None
                    if watcha['ret'] == 'success':
                        tmdb['data']['country'] = watcha['data'][0]['country']
                        if watcha['data'][0]['country'] == []:
                            try:
                                tmdb['data']['country'] = watcha['data'][0]['seasons'][0]['info']['country']
                            except:
                                tmdb['data']['country'] == None
            return tmdb
        except Exception as e:
    ...
    def check_kor_sub(data, tmdb, sub_o_path):
        ...
        try:
            if data['guessit']['release_group'] in ['SW', 'ST']:
                logger.debug('VOD 파일')
                return True
            elif os.path.isfile(os.path.splitext(data['fullPath'])[0]+'.ko.srt'):
                ...
                return True  
            elif os.path.isfile(os.path.splitext(data['fullPath'])[0]+'.smi'):
                logger.debug('외부 자막 있음') 
                LogicNormal.file_move(os.path.splitext(data['fullPath'])[0]+'.smi',data['dest'],os.path.splitext(data['name'])[0]+'.smi')
                return True 
            elif os.path.isfile(os.path.splitext(data['fullPath'])[0]+'.srt'):
                logger.debug('외부 자막 있음') 
    ...
    def genre_move(data, tmdb, sub_o_path):
        ...
        if tmdb is None:
            if ModelSetting.get_bool('sub_x_flag'):
                ...
                LogicNormal.file_move(data['fullPath'],data['dest'],data['name'])
                LogicNormal.db_save(data['fullPath'], data['dest'], '불일치', True)
            else:
    ...
    def move_ftv(data, tmdb, sub_o_path):
        try:
            ...
            output = subprocess.Popen(command, stderr=subprocess.STDOUT, stdout=subprocess.PIPE, encoding='utf-8')
            ffprobe_json = json.dumps(output.communicate())
            if '1920' in ffprobe_json or '2160' in ffprobe_json:
                if ModelSetting.get_bool('uhd_flag'):
                    logger.debug('UHD 파일')
                    try:
    ...
    @staticmethod
    def db_save(fullPath, dest, match_type, is_moved):
        # telegram_flag = ModelSetting.get_bool('telegram')
        try:
            entity = {}
            entity['name'] = guessit(os.path.basename(fullPath))['title']
            entity['fileName'] = os.path.basename(fullPath)
            entity['dirName'] = os.path.dirname(fullPath)
